...
export interface NetworkObject {
  ...
  // Token types
  export type MetaType = 'SUPPLIED' | 'BORROWED' | 'CLAIMABLE' | 'VESTING' | 'LOCKED' | 'NFT' | 'WALLET';
  export type TokenType = 'base-token' | 'app-token' | 'contract-position' | 'non-fungible';
  export type NetworkString = 'ETHEREUM_MAINNET' | 'BASE_MAINNET' | 'POLYGON_MAINNET' | 'ARBITRUM_MAINNET' | string;
  ...
  type AbstractToken = BaseTokenPositionBalance | AppTokenPositionBalance | NonFungiblePositionBalance;
  ...
  function parseTokens(tokens: AbstractToken[] | undefined, level: number = 1, maxLevel: number = 3): ParsedToken[] {
    ...
    return tokens.map(token => {
      ...
      if (token.type === 'app-token') {
        ...
        const appToken: AppToken = {
          ...
          ...(level < maxLevel && appTokenData.tokens && { underlyingTokens: parseTokens(appTokenData.tokens, level + 1, maxLevel) })
        };
        return appToken;
      }
  ...
  function parseContractTokens(tokens: TokenWithMetaType[] | undefined, level: number = 1, maxLevel: number = 3): ContractToken[] {
    if (!tokens || level > maxLevel) return [];
    ...
    if (appBalances.byApp?.edges) {
      ...
      parsed.apps = appBalances.byApp.edges.map(edge => {
        ...
        const parsedApp: ParsedApp = {
          ...
          positions: (app.positionBalances?.edges || app.balances?.edges || [])
            .map(posEdge => parsePositionBalance(posEdge.node))
        };
  ...
  export function getPositionsByProtocol(parsedAppBalances: ParsedAppBalances): Record<string, ProtocolPosition> {
  ...
  export function getAllTokensFlat(parsedAppBalances: ParsedAppBalances): Array<ParsedToken & { appName: string; networkObject: NetworkObject }> {
