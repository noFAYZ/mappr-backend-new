...
export const DeFiFiltersSchema = z.object({
  protocols: z.array(z.string().min(1).max(50))
    .max(20, 'Maximum 20 protocols allowed')
    .optional(),
  
  types: z.array(z.string().min(1).max(30))
    .max(10, 'Maximum 10 position types allowed')
    .optional(),
...
export const AssetFiltersSchema = z.object({
  ...
  minBalance: z.coerce.number()
    .min(0, 'Minimum balance must be non-negative')
    .optional(),
  
  maxBalance: z.coerce.number()
    .min(0, 'Maximum balance must be non-negative')
    .optional(),
...
export const AnalyticsQuerySchema = z.object({
  ...
  metrics: z.array(z.enum([
    ...
    'stakingRewards',
    'gasSpent'
  ])).min(1, 'At least one metric must be specified').optional(),
...
export const ExportRequestSchema = z.object({
  ...
  dataTypes: z.array(z.enum(['transactions', 'assets', 'nfts', 'defi']))
